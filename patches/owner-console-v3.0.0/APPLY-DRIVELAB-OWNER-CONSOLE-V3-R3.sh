...
TEMPORARY="$(mktemp /tmp/APPLY-DRIVELAB-OWNER-CONSOLE-V3-R3.XXXXXX.sh)"
...
chmod 0755 "$WORK"
chown -R drivelab-license:drivelab-license "$WORK/pycache" "$STAGE"
chmod 0755 "$STAGE" "$STAGE/drivelab_license"
...
''',
    ...
    case "\\$name" in
        drivelab-owner-test-runner.py|drivelab-owner-ops-worker) destination="/usr/local/sbin/\\$name" ;;
''',
    "rollback file list",
)
